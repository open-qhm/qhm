	...
	$.fn.prettyEmbed = function (options) {
		...
		if (mobile) {
			$(window).on('load', function() {
				$('.pretty-embed:visible').trigger('click');

				$('.pretty-embed:hidden').each(function(){
					var self = this
					// the video is descendent of bootstrap collapsible element
					var collapsed = $(self).closest('.collapse').one('shown.bs.collapse', function(){
						$(self).trigger('click');
					});

					if (collapsed.length === 0) {
						// the video is descendent of bootstrap tab-pane element
						var tabbed = $(self).closest('.tab-pane');
						var expanded = false;
						if (tabbed.length > 0) {
							$('#tab1').on('shown.bs.tab', function(e){
								if (!expanded && $(e.target).attr('href').substr(1) === $(tabbed).attr('id')) {
									expanded = true;
									$(self).trigger('click');
								}
							})
