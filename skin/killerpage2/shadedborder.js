...
create: function(opts) {
  ...
  function op(v) {
	...
	//customize hokuken
	var div_height = opts.mwheight * 1.5 +2048;
  ...
  function mid(mw) {
    var ds = [];
    var max_height = 30000; //unsiged int max value

	//customeize hokuken
    ds.push('<div style="position:relative; top:' + (th+bh) + 'px; height:'+div_height+'px; ' +
            ' margin:0 ' + (rw-r-cx) + 'px 0 ' + (lw-r-cx) + 'px; ' +
            ' padding:0; overflow:hidden;' +
            ' background-position:0 ' + (th > 0 ? -(r+cy+cs) : '0') + 'px;"' +
            ' class="' + iclass + '"></div>');

	//customeized by hokuken
	
	var isie = /msie/i.test(navigator.userAgent) && !window.opera;
	
	if(isie && div_height > max_height){
//	if(true){
		var total_h = div_height;
		var tmp_h = 0;
		var tmp_top = th+bh;
		
		while( total_h > 0 ) {
			
			tmp_h = (total_h > max_height) ? max_height : total_h;
			total_h -= max_height;
		
		    var dd = '<div style="position:absolute; width:1px;' +
		        ' top:' + (tmp_top) + 'px; height:'+tmp_h+'px; padding:0; margin:0;';
		    if (sr>0) {
		      for (var x=0; x<lw-r-cx; ++x) { //left QQQ
		        ds.push(dd + ' left:' + x + 'px;' + op((x+1.0)/lw) + 
		            '" class="' + sclass + '"></div>');
		      }
		
		      for (var x=0; x<rw-r-cx; ++x) { //right
	...
	else{ //------------ normal processing start here -----------------
	
    var dd = '<div style="position:absolute; width:1px;' +
        ' top:' + (th+bh) + 'px; height:'+div_height+'px; padding:0; margin:0;';
    if (sr>0) {
      for (var x=0; x<lw-r-cx; ++x) { //left QQQ
        ds.push(dd + ' left:' + x + 'px;' + op((x+1.0)/lw) + 
            '" class="' + sclass + '"></div>');
